...
## Plot cone ##
## Parameters:
# cl: rbprm-corbaserver client (rbprmBuilder.client.rbprm.rbprm)
...
def plotPointJointPath (r, cl, robot, dt, nPath, jointName, pointInJoint, pathName, pathColor):
    points = []
    for t in np.arange(0., cl.problem.pathLength(nPath), dt):
...
def plotJointFrame (r, cl, q, jointName, ampl):
    cl.robot.setCurrentConfig (q)
    jointGlobTransf = cl.robot.getJointPosition (jointName)
    frameOrigin = cl.robot.computeGlobalPosition (jointGlobTransf, [0,0,0])
    framePositionX = cl.robot.computeGlobalPosition (jointGlobTransf, [ampl,0,0])
    framePositionY = cl.robot.computeGlobalPosition (jointGlobTransf, [0,ampl,0])
    framePositionZ = cl.robot.computeGlobalPosition (jointGlobTransf, [0,0,ampl])
...
def plotVectorInJointFrame (r, cl, q, jointName, vector, lineColor):
    cl.robot.setCurrentConfig (q)
    jointPosition = cl.robot.getJointPosition (jointName)
    framePosition = cl.robot.computeGlobalPosition (jointPosition, [0,0,0]) 
    extrPosition = cl.robot.computeGlobalPosition (jointPosition, [vector[0],vector[1],vector[2]])
